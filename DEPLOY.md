...
### Варианты авторизации (выберите один):

#### Вариант 1: Авторизация по паролю (проще для reg.ru)

- **`DEPLOY_PASSWORD`** - Пароль от SSH пользователя
  - Для reg.ru: используйте пароль от вашего аккаунта хостинга
  - **Важно:** Workflow автоматически установит `sshpass` и будет использовать пароль

#### Вариант 2: Авторизация по SSH ключу (более безопасно)

- **`DEPLOY_SSH_KEY`** - Приватный SSH ключ (содержимое файла `~/.ssh/deploy_key`)
  - Скопируйте весь ключ включая строки `-----BEGIN ... KEY-----` и `-----END ... KEY-----`
  - **Важно:** Публичный ключ должен быть добавлен в `~/.ssh/authorized_keys` на сервере

**Рекомендация для reg.ru:** Используйте вариант с паролем (`DEPLOY_PASSWORD`), так как на shared хостинге настройка SSH ключей может быть сложнее.
...
**Ошибка:** `Connection closed by *** port 22` или `Permission denied`

**Решение:**

1. **Если используете пароль (`DEPLOY_PASSWORD`):**
   - Убедитесь, что пароль правильный
   - Проверьте, что SSH доступ включен в панели управления reg.ru
   - Убедитесь, что `DEPLOY_PASSWORD` добавлен в GitHub Secrets

2. **Если используете SSH ключ (`DEPLOY_SSH_KEY`):**
   ...
   **Проверка на сервере:**
   ...
   # Проверьте наличие authorized_keys
   ...
   **Проверка ключа в GitHub Secrets:**
   - Убедитесь, что в `DEPLOY_SSH_KEY` скопирован **приватный** ключ (не публичный!)
   - Ключ должен начинаться с `-----BEGIN OPENSSH PRIVATE KEY-----` или `-----BEGIN RSA PRIVATE KEY-----`
   - Ключ должен заканчиваться на `-----END ... KEY-----`
   - Убедитесь, что скопированы все строки, включая начало и конец
   ...
   **Проверка публичного ключа на сервере:**
   ...
   # Скопируйте вывод и проверьте на сервере
   # На сервере должно быть в ~/.ssh/authorized_keys
   grep "ваш_публичный_ключ" ~/.ssh/authorized_keys
   ```
...
3. **Общие проверки:**
   ...
   - Убедитесь, что SSH доступ включен в панели управления reg.ru
   - Проверьте, что сервер не блокирует подключения с IP адресов GitHub Actions

4. **Рекомендация:**
   Если SSH ключ не работает, переключитесь на авторизацию по паролю:
   - Добавьте `DEPLOY_PASSWORD` в GitHub Secrets
   - Удалите или оставьте пустым `DEPLOY_SSH_KEY`
